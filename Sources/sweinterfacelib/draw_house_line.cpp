...
LineXY3 *DrawHouseLines::lines(H *house) {
    ...
    for (int i = 0; i < MAX_HOUSE; ++i) {
        ...
        if (house[i].angle != ANGLES_ASC && house[i].angle != ANGLES_FC &&
            house[i].angle != ANGLES_DESC & house[i].angle != ANGLES_MC) {
...
LineXY3 *DrawHouseLines::triangles_small(H *house) {
    const int MAX_HOUSE = 12;
    const double ANGULAR_POINTER = -0.7;
    const double TRIANGLE_TOP = 78;
    const double TRIANGLE_BOTTOM = 76;
    LineXY3 *res = new LineXY3[MAX_HOUSE];
    for (int i = 0; i < MAX_HOUSE; ++i) {
        double off_house = 360 - house[0].longitude;
...
LineXY DrawHouseLines::angle_lines(H *house, int angle) {
