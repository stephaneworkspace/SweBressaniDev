//
// Created by Stéphane on 22.09.22.
//
#include "include/sweinterfacelib.h"

using namespace sweinterfacelib;

// aqua blue dark 009FFF

const char* SIGNS_BELIER_SVG = "";
const char* SIGNS_BELIER_DARK_SVG = SIGNS_BELIER_SVG;
const char* SIGNS_TAUREAU_SVG = "";
const char* SIGNS_TAUREAU_DARK_SVG = SIGNS_TAUREAU_SVG;
const char* SIGNS_GEMAUX_SVG = "";
const char* SIGNS_GEMAUX_DARK_SVG = SIGNS_GEMAUX_SVG;
const char* SIGNS_CANCER_SVG = "";
const char* SIGNS_CANCER_DARK_SVG = "";
const char* SIGNS_LION_SVG = "";
const char* SIGNS_LION_DARK_SVG = SIGNS_LION_SVG;
const char* SIGNS_VIERGE_SVG = "";
const char* SIGNS_VIERGE_DARK_SVG = SIGNS_VIERGE_SVG;
const char* SIGNS_BALANCE_SVG = "";
const char* SIGNS_BALANCE_DARK_SVG = SIGNS_BALANCE_SVG;
const char* SIGNS_SCORPION_SVG = "";
const char* SIGNS_SCORPION_DARK_SVG = "";
const char* SIGNS_SAGITTAIRE_SVG = "";
const char* SIGNS_SAGITTAIRE_DARK_SVG = SIGNS_SAGITTAIRE_SVG;
const char* SIGNS_CAPRICORNE_SVG = "";
const char* SIGNS_CAPRICORNE_DARK_SVG = SIGNS_CAPRICORNE_SVG;
const char* SIGNS_VERSEAU_SVG  = "";
const char* SIGNS_VERSEAU_DARK_SVG = SIGNS_VERSEAU_SVG;
const char* SIGNS_POISSON_SVG  = "";
const char* SIGNS_POISSON_DARK_SVG = "";

const string Sign::read_svg(int sign, int color_mode) {
    const string r = "data:image/svg+xml;base64,";
    if (color_mode == COLOR_MODE_LIGHT) {
        switch (sign) {
            case SIGNS_BELIER:
                return r + SIGNS_BELIER_SVG;
            case SIGNS_TAUREAU:
                return r + SIGNS_TAUREAU_SVG;
            case SIGNS_GEMAUX:
                return r + SIGNS_GEMAUX_SVG;
            case SIGNS_CANCER:
                return r + SIGNS_CANCER_SVG;
            case SIGNS_LION:
                return r + SIGNS_LION_SVG;
            case SIGNS_VIERGE:
                return r + SIGNS_VIERGE_SVG;
            case SIGNS_BALANCE:
                return r + SIGNS_BALANCE_SVG;
            case SIGNS_SCORPION:
                return r + SIGNS_SCORPION_SVG;
            case SIGNS_SAGITTAIRE:
                return r + SIGNS_SAGITTAIRE_SVG;
            case SIGNS_CAPRICORNE:
                return r + SIGNS_CAPRICORNE_SVG;
            case SIGNS_VERSEAU:
                return r + SIGNS_VERSEAU_SVG;
            case SIGNS_POISSON:
                return r + SIGNS_POISSON_SVG;
            default:
                return "";
        }
    } else {
        switch (sign) {
            case SIGNS_BELIER:
                return r + SIGNS_BELIER_DARK_SVG;
            case SIGNS_TAUREAU:
                return r + SIGNS_TAUREAU_DARK_SVG;
            case SIGNS_GEMAUX:
                return r + SIGNS_GEMAUX_DARK_SVG;
            case SIGNS_CANCER:
                return r + SIGNS_CANCER_DARK_SVG;
            case SIGNS_LION:
                return r + SIGNS_LION_DARK_SVG;
            case SIGNS_VIERGE:
                return r + SIGNS_VIERGE_DARK_SVG;
            case SIGNS_BALANCE:
                return r + SIGNS_BALANCE_DARK_SVG;
            case SIGNS_SCORPION:
                return r + SIGNS_SCORPION_DARK_SVG;
            case SIGNS_SAGITTAIRE:
                return r + SIGNS_SAGITTAIRE_DARK_SVG;
            case SIGNS_CAPRICORNE:
                return r + SIGNS_CAPRICORNE_DARK_SVG;
            case SIGNS_VERSEAU:
                return r + SIGNS_VERSEAU_DARK_SVG;
            case SIGNS_POISSON:
                return r + SIGNS_POISSON_DARK_SVG;
            default:
                return "";
        }
    }
}

const char* Sign::read_svg_c(int sign, int color_mode) {
    if (color_mode == COLOR_MODE_LIGHT) {
        switch (sign) {
        case SIGNS_BELIER:
            return SIGNS_BELIER_SVG;
        case SIGNS_TAUREAU:
            return SIGNS_TAUREAU_SVG;
        case SIGNS_GEMAUX:
            return SIGNS_GEMAUX_SVG;
        case SIGNS_CANCER:
            return SIGNS_CANCER_SVG;
        case SIGNS_LION:
            return SIGNS_LION_SVG;
        case SIGNS_VIERGE:
            return SIGNS_VIERGE_SVG;
        case SIGNS_BALANCE:
            return SIGNS_BALANCE_SVG;
        case SIGNS_SCORPION:
            return SIGNS_SCORPION_SVG;
        case SIGNS_SAGITTAIRE:
            return SIGNS_SAGITTAIRE_SVG;
        case SIGNS_CAPRICORNE:
            return SIGNS_CAPRICORNE_SVG;
        case SIGNS_VERSEAU:
            return SIGNS_VERSEAU_SVG;
        case SIGNS_POISSON:
            return SIGNS_POISSON_SVG;
        default:
            return "";
        }
    } else {
        switch (sign) {
            case SIGNS_BELIER:
                return SIGNS_BELIER_DARK_SVG;
            case SIGNS_TAUREAU:
                return SIGNS_TAUREAU_DARK_SVG;
            case SIGNS_GEMAUX:
                return SIGNS_GEMAUX_DARK_SVG;
            case SIGNS_CANCER:
                return SIGNS_CANCER_DARK_SVG;
            case SIGNS_LION:
                return SIGNS_LION_DARK_SVG;
            case SIGNS_VIERGE:
                return SIGNS_VIERGE_DARK_SVG;
            case SIGNS_BALANCE:
                return SIGNS_BALANCE_DARK_SVG;
            case SIGNS_SCORPION:
                return SIGNS_SCORPION_DARK_SVG;
            case SIGNS_SAGITTAIRE:
                return SIGNS_SAGITTAIRE_DARK_SVG;
            case SIGNS_CAPRICORNE:
                return SIGNS_CAPRICORNE_DARK_SVG;
            case SIGNS_VERSEAU:
                return SIGNS_VERSEAU_DARK_SVG;
            case SIGNS_POISSON:
                return SIGNS_POISSON_DARK_SVG;
            default:
                return "";
        }
    }
}

const string Sign::nom(int sign) {
    switch (sign) {
        case SIGNS_BELIER:
            return "Belier";
        case SIGNS_TAUREAU:
            return "Taureau";
        case SIGNS_GEMAUX:
            return "Gemaux";
        case SIGNS_CANCER:
            return "Cancer";
        case SIGNS_LION:
            return "Lion";
        case SIGNS_VIERGE:
            return "Vierge";
        case SIGNS_BALANCE:
            return "Balance";
        case SIGNS_SCORPION:
            return "Scorpion";
        case SIGNS_SAGITTAIRE:
            return "Sagittaire";
        case SIGNS_CAPRICORNE:
            return "Capricorne";
        case SIGNS_VERSEAU:
            return "Verseau";
        case SIGNS_POISSON:
            return "Poisson";
        default:
            cerr << "Unreachable code reached" << endl;
            return "";
    }
}